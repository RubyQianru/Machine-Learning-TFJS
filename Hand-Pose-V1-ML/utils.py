...
def load_data(file_paths, random_seed=1):
    ...
    for file_path in file_paths:
        ...
        y_temp = [sample['ys'] for sample in json_data['data']]
    ...
    x_data = np.array([list(sample.values()) for sample in all_x_data])
    ...
    y_data = np.array(all_y_data)[shuffle_indices]
...
def preprocess_data(x_train, y_train, x_test, y_test):
    ...
    x_train = x_train.reshape(-1, 63).astype("float32") / 640
    x_test = x_test.reshape(-1, 63).astype("float32") / 640
